...
(function (scope) {
  ...
  p.init = function (scope, options) {
    if (!options) {
      this.doPreventScroll = this.detectScrollPrevention =
        options.doPreventScroll;
    }
  ...
  p._handleTouchStart = function (event) {
    ...
    this.swipeStart_clientX = touches
      ? event.touches[0].clientX
      : event.clientX;
    this.swipeStart_clientY = touches
  ...
  p._handleTouchMove = function (event) {
    if (this.detectScrollPrevention) {
      ...
      var diffX =
        ...
        (touches ? event.touches[0].clientX : event.clientX);
      var diffY =
        this.swipeStart_clientY -
        (touches ? event.touches[0].clientY : event.clientY);
    ...
    if (this.doPreventScroll && this.continuePreventingScroll) {
  ...
  p._handleTouchEnd = function (event) {
    ...
    // check for the TouchList before getting X/Y data
    var touches = !!event.touches;
    this.swipeEnd_clientX = touches
      ? event.changedTouches[0].clientX
      : event.clientX;
    this.swipeEnd_clientY = touches
